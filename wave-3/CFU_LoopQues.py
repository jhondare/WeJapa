...
#lets create our loop

for prime_num in check_prime:
    for i in range(2, prime_num):
        if (prime_num % i == 0):
            print(prime_num)
            """
            print('current index::{}'.format(i))
            print('current prime number::{}'.format(prime_num))
            print('this is the modulo remainder::{}'.format(prime_num%i))
            print('it terminates here . . . . moving on to the next index:{}'.format(i+1))
            """
        #else: 
            #print('{} has remainder for {}, which is: {}'.format(i, prime_num, prime_num % i))
